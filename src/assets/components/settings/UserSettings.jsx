...
function UserSettings({ triggerBack }) {
  ...
  useEffect(() => {
    const isChange = username !== currentUser.displayName || profile.length > 0
  ...
  const updateProfileImage = async () => {
    ...
    const storageRef = ref(
      storage,
      `profiles/user-${currentUser.uid}.${extension}`
    )
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (isChanged) {
        const msg =
          "Changes have not been saved, do you want to exit this page session? This action cannot be undone."
        e.returnValue = msg
      }
    }
    ...
    <div className="settings user-settings" onSubmit={handleSave}>
      ...
      <form className="settings-form">
        ...
        <section className="settings-item">
          ...
          <div className="desc">
            Share with friends, companions, services, anyone to communicate with
            you
          </div>
        </section>
        <section className="settings-item">
        ...
        <section className="settings-item">
          <button type="submit" disabled={!isChanged && btnState == "idle"}>
            {btnState == "idle" ? "Save Changes" : <LoadingAnim />}
          </button>
        </section>
      </form>
    </div>
